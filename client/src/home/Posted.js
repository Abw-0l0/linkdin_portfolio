    ...
    <div className="flex h-fit border-[1px] rounded-lg mb-2 pt-2 pb-1 w-full flex-col bg-white shadow-sm">
      <div className="px-3 h-1/2 flex flex-row w-full items-center">
        ...
        <div className="flex text-gray-600 rounded-md h-full w-fit ">
          <MoreHorizIcon
            onClick={() => setPostDelToggle(!postDelToggle)}
            ...
            <div className="relative">
              <div className="absolute bg-white right-0 mt-7 border-[2px] shadow-md w-fit h-fit rounded-lg">
                <p
                  ...
                  className="pr-7 pl-3 py-2 hover:bg-gray-100 cursor-pointer text-sm font-medium text-gray-700"
                >
                  Delete
                </p>
              </div>
      ...
      <div className="flex flex-col w-full pb-1">
        <div className="px-3 flex pb-2">
          <p className="text-sm text-gray-600 font-normal">{post.caption}</p>
        </div>
        <div className="flex w-full object-cover justify-center max-h-[670px]">
          <img
            className="flex"
            src={post.photo !== "" ? `${URL}/uploads/` + post.photo : ""}
            alt=""
          />
        </div>
      </div>
      ...
      <div className="px-3 flex flex-row w-full border-b-[1px] py-1 justify-between">
        <div className="space-x-1 flex w-fit flex-row">
          <ThumbUpOffAltTwoToneIcon
            className="text-gray-700 bg-blue-300 p-[1px] rounded-full"
            fontSize="small"
          />
          <p className="text-xs text-gray-400">{post.likes.length}</p>
        </div>
        <div className="space-x-1 flex w-fit flex-row">
          <p className="text-xs text-gray-400 hover:underline hover:text-blue-500 cursor-pointer">
      ...
      <div className="px-3 flex w-full flex-row mt-1 space-x-1 justify-around">
        <div
          onClick={addLike}
          className="flex flex-row items-center w-full justify-center cursor-pointer hover:bg-gray-100 px-2 py-2 rounded-md"
        >
          <Like
            className="text-gray-500"
            fontSize="medium"
            color={`${
              post.likes.some((like) => like.user._id === user.user.userId)
                ? "primary"
                : ""
            }`}
          />
