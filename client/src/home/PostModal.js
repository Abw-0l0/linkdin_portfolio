...
function PostModal({caption, setCaption,inputKey,setModal,modal,inputRef,handleSubmit,image,setImage}) {
        ...
        <div className='flex justify-center w-full h-full'>
            <div className='bg-white w-1/2 mt-8 z-50 h-3/4 rounded-xl overflow-hidden flex flex-col'>
                ...
                <div className='flex flex-col w-full h-3/5 overflow-y-scroll'>
                    ...
                    <div id="parentDiv" className="mb-2 px-5 flex flex-shrink min-h-[150px] w-full">
                        <textarea
                        className='resize-none w-full h-full overflow-hidden outline-none text-gray-900 text-xl' 
                        type="text" placeholder="What do you want to talk about?" id="myTextarea" 
                        value={caption} onChange={handleChange}/>
                    </div>

                    {image && (
                        <div className='flex flex-col p-5'>
                            <p onClick={removeImage} className='self-end m-1 z-20 flex text-gray-500 bg-gray-200 hover:bg-gray-300 p-1 px-3 rounded-full w-fit h-fit font-medium cursor-pointer text-xl'>X</p>
                            <img src={image} alt="Selected" className='w-full z-10 -mt-11 h-auto rounded-lg border-[1px]' />
                        </div>
                    )}
                ...
                <div className='flex flex-col w-full h-1/5'>
                    <div className='flex flex-row space-x-7 w-full h-1/2 items-center px-5'>
                        <ImageIcon onClick={handleIconClick} className='text-gray-500 cursor-pointer hover:text-gray-700' fontSize='medium'/>
                        <Event className='text-gray-500 cursor-pointer hover:text-gray-700' fontSize='medium'/>
                        <CakeIcon className='text-gray-500 cursor-pointer hover:text-gray-700' fontSize='medium'/>
                        <MoreHorizIcon className='text-gray-500 cursor-pointer hover:text-gray-700' fontSize='medium'/>
                    </div>
